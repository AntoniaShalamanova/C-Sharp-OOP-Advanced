    ...
    public class FlightController : IFlightController
	{
		...
		public string TakeOff()
		{
			...
			foreach (var trip in this.airport.Trips)
			{
                ...
                //TestLoadCarryOnBaggage
                LoadCarryOnBaggage(trip);

                //TestIfAirplaneIsOverbooked
				if (trip.Airplane.IsOverbooked)
				{
					IPassenger[] ejectedPassengers = EjectOverbookedPassengers(trip).ToArray();
		...
		{
			IAirplane plane = trip.Airplane;

			foreach (var passenger in plane.Passengers)
			{
				List<IBag> luggage = passenger.Bags.ToList();
